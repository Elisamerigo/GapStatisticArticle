...
#First Cluster
for (j in 1:ncol(clusterOriginal_df)) {
...
#Second cluster
for (k in 1:ncol(clusterOriginal_df2)) {
...
#Calculate k-Means and Gap Statistic
...
data1<-data.frame(clusterOriginalAll, GapValues=round(gap_stat1$gap, 4))

#Create a loop to create eight cases 
for(i in 1:8){
...
data2 <- data.frame(df_finale, GapValues=rep(gap_ind, each=24))
...
Gap<- ifelse(dataFinale$GapValues >= 0, paste0("+", format(round(dataFinale$GapValues, 4), nsmall = 4)), 
                  format(round(dataFinale$GapValues, 4), nsmall = 4))
dataFinaleForm<-data.frame(dataFinale, Gap)

#Finale graphic
ggplot(dataFinaleForm, aes(x = x, y = y, color=GapValues)) +
  geom_point(size=1) +
  facet_wrap( ~Gap, labeller=label_both)+
  theme(strip.text.x = element_text(size = 10, vjust = 0.5))+
