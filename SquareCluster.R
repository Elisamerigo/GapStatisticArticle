...
gap_stat<- format(round(gap_stat$gap, 4))
...
ggplot() +
  ...
  labs(title = paste0("k-Means with k = 2, Gap = ", gap_stat), x = "X", y = "Y") +  # Aggiungi titolo e etichette
  theme(plot.title = element_text(hjust = 0.5))+  # Centra il titolo
  xlim(-2, 15) +  # Imposta i limiti per l'asse X
  ylim(-2, 15)  
...
#Graph with ggplot2. Clusters with centroids
ggplot()+
  geom_point(data=cluster_df, aes(x = x, y = y), color = "blue", size = 3) +# Disegna i punti
  geom_point(data=cluster_df2, aes(x = x, y = y), color = "red", size = 3) +
