...
data1<-data.frame(clusterOriginalAll, GapFirst=round(gap_stat1$gap, 4))
...
data2 <- data.frame(df_finale, GapFirst=rep(gap_ind, each=24))
...
Gap<- ifelse(dataFinale$Gap >= 0, paste0("+", format(round(dataFinale$Gap, 4), nsmall = 4)), 
                  format(round(dataFinale$Gap, 4), nsmall = 4))
dataFinaleForm<-data.frame(dataFinale, Gap)

#Finale graphic
ggplot(dataFinaleForm, aes(x = x, y = y, color=GapFirst)) +
  geom_point(size=1) +
